...
## Overview
SCOPE is organized around a chemistry model, a workflow model, an analysis model, and a runtime model for HPC submission. The core package handles workflows for generic molecules and molecule-based crystals, while `azo` and `sco` add domain-specific behavior.
...
Normally, the user works with the main classes (Molecule and Cell), while substructures are used internally by SCOPE to execute tasks. Those classes hold structural data, connectivity, and chemical metadata used by later workflow and analysis stages. They must be sourced to a System in order to execute a computational workflow. There are System-class functions to facilitate this task. Species can be created from `.xyz` data, RDKit molecules, or cell2mol molecules. Cells can be created from `.xyz` data and cell parameters, or cell2mol Cell objects.
...
2. create a `System`.
3. add sources (`Molecule` or `Cell`). The easiest way is to define a set of labels and coordinates, and create a molecule with them.
