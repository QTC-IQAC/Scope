    ...
    with open(outfolder+filename, "w") as output_file:    
        for f in range(-maxfactor,maxfactor,1):
            ...
            for idx in range(natoms):
                ...
                pos = geom[idx]+vector*f*0.1
                label = elemdatabase.elementsym[vnm.atnums[idx]]
                print(f"{label}  {pos[0]:.5f}  {pos[1]:.5f}  {pos[2]:.5f}", file=output_file)

def vnm_displacement(VNMs: list, initial_coord: list, which: list=[], which_side: str='pos', maxfactor: int=10, debug: int=0):
    ...
    for vnm in VNMs:
        if vnm.freq < 0.0 and vnm.index in which:
            ...
            for idx in range(len(vnm.xs)):
                vector = np.array([vnm.xs[idx], vnm.ys[idx], vnm.zs[idx]])
                if which_side.lower() == 'pos': displacement = vector*maxfactor*0.1
                elif which_side.lower() == 'neg': displacement = -vector*maxfactor*0.1
