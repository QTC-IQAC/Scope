###import sys
###
###scopepath = '/home/g4vela/SCOPE/Database_SCO/Scripts/Scope'
###sys.path.append(scopepath)

from .Adapted_from_cell2mol import *
from .Constants import *
from .Classes_QC import *
from .Classes_Job import *
from .Classes_SCO import *
from .Control_Jobs import *
from .Geom_SCO_V1 import *
from .Other import *
from .Parse_Cif import *
from .Parse_General import *
from .Generate_Representations import *
from .Gmol_ops import *
from .Parse_G16_outputs import * 
from .Parse_QE_outputs import *
from .Read_Write import *
from .Register_Data import *
from .Thermal_Corrections import *
from .unit_cell_tools import *
from .VNM_tools import *
from .Write_G16_Inputs import *
from .Write_QE_Inputs import *

#from . import Constants 
#from . import Adapted_from_cell2mol 
#from . import Control_Jobs 
#from . import Geom_SCO_V1
#from . import Other 
#from . import Parse_Cif 
#from . import Parse_General 
#from . import Gmol_ops 
#from . import Parse_G16_outputs 
#from . import Parse_QE_outputs 
#from . import Read_Write 
#from . import Scope_Classes 
#from . import Scope_Logistics 
#from . import Thermal_Corrections 
#from . import unit_cell_tools 
#from . import VNM_tools 
#from . import Write_G16_Inputs 
