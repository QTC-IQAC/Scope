...
class System_azo(System):
    ...
    def get_dihedral_indices(self):
        ...
        return list([at0, at1, at2, at3, at4, at5]), fragments
    ...
    def create_ts(self, ts_list:list = ['TSrot', 'TSinv', 'triplet'], debug: int=0):
        ...
        They can be accessed using the keys 'TSrot', 'TSrot_B', 'TSinv_L', 'TSinv_R', 'TSrot_A_T', 'TSrot_B_T'.
        ...
        Nomenclature: 
            - TSrot_A_S: Rotation TS in singlet state. Dihedral angle is set as +90º. Total spin is set as 0 (by default).
            - TSrot_A_T: Rotation TS in triplet state. Dihedral angle is set as +90º. Total spin is set as 2. 
            - TSrot_B_S: Rotation TS in singlet state. Dihedral angle is set as -90º. Total spin is set as 0 (by default).
        ...
        Two versions are created:
            - TSinv_L: Inversion TS involving inversion of left ring (at0 - at1 = at2). Total spin is set as 0.
            - TSinv_R: Inversion TS involving inversion of right ring (at1 = at2 - at3). Total spin is set as 0.
            ...
            if found:
                ...
                if not isFragmented:
                    ...
                    if 'triplet' in ts_list:
                        ...
                        ts_triplet.set_total_spin(2)    # Number of unpaired electrons 
        ...
        if 'TSinv' in ts_list:
            ...
            if debug > 0: 
                ...
                print(f'AZOS.CREATE_TS.TSINV_L_A: Angle between {at1}, {at2} and {at3} set to {angle_deg} degrees.')
            ...
            for a0 in angles:
                ...
                if is_equal:
                    ts = Molecule_azo(labels, coord)
                    ts_isFragmented = ts.check_fragmentation()  # Check if the TSinv_l_a is fragmented
                    if not ts_isFragmented:
                        ...
                        created_ts.append(ts)
                        if debug > 0: print(f'AZOS.CREATE_TS.TSINV_L_A: TSinv_l_a Molecule_azo successfully created for {self.name}')
                        break
                    else:
                        raise Exception(f'AZOS.CREATE_TS.TSINV_L_A: [ERROR] TSinv_l_a fragmented for {self.name}')
        ...
        if 'TSinv' in ts_list:
            ...
            angles = np.concatenate(([0],[val for i in range(1, 12) for val in (15 * i, -15 * i)]))
            angles = np.insert(angles[::-1],0,180.)     # Starts from 180 degrees
            for a0 in angles:
                coord = set_dihedral(labels, coord, a0, at3,at2,at1,at0, adjmat=adjmat_ref, adjnum=adjnum_ref)
                _, adjmat, adjnum = get_adjmatrix(labels,coord)
                is_equal = np.array_equal(adjmat, adjmat_ref) and np.array_equal(adjnum, adjnum_ref)
                if is_equal:
                    ts = Molecule_azo(labels, coord)
                    ts_isFragmented = ts.check_fragmentation()  # Check if the TSinv_l_b is fragmented
                    if not ts_isFragmented:
                        ...
                        created_ts.append(ts)
                        if debug > 0: print(f'AZOS.CREATE_TS.TSINV_L: TSinv_l_b Molecule_azo successfully created for {self.name}')
                        break
                    else:
                        raise Exception(f'AZOS.CREATE_TS.TSINV_L: [ERROR] TSinv_l_b fragmented for {self.name}')
        ...
        if 'TSinv' in ts_list:
            ...
            if debug > 0: 
                ...
                print(f'AZOS.CREATE_TS.TSINV_R_A: Angle between {at2}, {at3} and {at4} set to {angle_deg} degrees.')
            angles = np.concatenate(([0],[val for i in range(1, 12) for val in (15 * i, -15 * i)]))
            for a0 in angles:
                ...
                if is_equal:
                    ...
                    if not ts_isFragmented:
                        ...
                        created_ts.append(ts)
                        if debug > 0: print(f'AZOS.CREATE_TS.TSINV_R: TSinv_r_a Molecule_azo successfully created for {self.name}')
                        break
                    else:
                        raise Exception(f'AZOS.CREATE_TS.TSINV_R: [ERROR] TSinv_r_a fragmented for {self.name}')
        if 'TSinv' in ts_list:
            coord = set_angle(labels, coord, 179.9, at2,at3,at4) ## Angle value of 179.9 instead of 180 to avoid numerical issues
            if debug > 0: 
                angle_deg = np.degrees(get_angle(coord[at3]-coord[at2], coord[at4]-coord[at2]))
            ...
            for a0 in angles:
                ...
                if is_equal:
                    ...
                    if not ts_isFragmented:
                        ...
                        created_ts.append(ts)
                        if debug > 0: print(f'AZOS.CREATE_TS.TSINV_R: TSinv_r_b Molecule_azo successfully created for {self.name}')
                        break
                    else:
                        raise Exception(f'AZOS.CREATE_TS.TSINV_R: [ERROR] TSinv_r_b fragmented for {self.name}')
        return created_ts
