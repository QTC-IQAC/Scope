...
def gen_G16_subfile(paths: object, procs: int=1, queue: str="iqtc09", cluster: str=set_cluster(), user: str=set_user(), savechk: bool=False):
    ...
    if 'login' in cluster or 'csuc' in cluster:
        with open(paths.sub_path, 'w+') as sub:
            print(f"#!/bin/bash", file=sub)
            print(f"#SBATCH -J {paths.name}{paths.suffix}", file=sub)
            print(f"#SBATCH -e /scratch/{user}/std_files/{paths.name}{paths.suffix}.stderr", file=sub)
            print(f"#SBATCH -o /scratch/{user}/std_files/{paths.name}{paths.suffix}.stdout", file=sub)
            ...
            print(f"cp $JOBDIR/{paths.inp_name} .", file=sub)
            print(f"echo '%nprocs={procs}' >  tmp1", file=sub)
            print(f"echo '%mem={mem}gb'    >> tmp1", file=sub)
            print(f"cat tmp1 {paths.inp_name} > tmp2 | mv -f tmp2 {paths.inp_name}", file=sub)
            print(f"g16 < {paths.inp_name} > {paths.out_name}", file=sub)
            print(f"cp -pr *.log $JOBDIR/", file=sub)
            if savechk: print(f"cp -pr *.chk $JOBDIR", file=sub)
            os.chmod(paths.sub_path, 0o777)

    elif 'portal' in cluster:
        with open(paths.sub_path, 'w+') as sub:
            print(f"#!/bin/bash", file=sub)
            print(f"#$ -N {paths.name}{paths.suffix}", file=sub) 
            print(f"#$ -pe smp {procs}", file=sub)
            print(f"#$ -cwd", file=sub)
            print(f"#$ -o /home/{user}/x-stds/{paths.name}{paths.suffix}.stdout", file=sub)
            print(f"#$ -e /home/{user}/x-stds/{paths.name}{paths.suffix}.stderr", file=sub)
            ...
            print(f"cd $TMPDIR", file=sub)
            print(f"export GAUSS_SCRDIR $TMPDIR", file=sub)
            print(f"cp $WORKDIR/{paths.inp_name} .", file=sub)
            print(f"echo '%nprocs={procs}' >  tmp1", file=sub)
            print(f"echo '%mem={mem}gb'    >> tmp1", file=sub)
            print(f"cat tmp1 {paths.inp_name} > tmp2 | mv -f tmp2 {paths.inp_name}", file=sub)
...
def gen_G16_input(mol: object, qc_data: object, paths: object, spin: str='gmol', debug: int=0):
    ...
    if spin.lower() == "gmol": 
        if hasattr(mol, 'spin'):
            spinval = 0
            for met in mol.metalist:
                if met.label == "Fe" and mol.spin.lower() == "ls": spinval = spinval + 1
                elif met.label == "Fe" and mol.spin.lower() == "hs": spinval = spinval + 5
            #spinval = mol.spin
        else:
            if ((mol.eleccount + mol.totcharge) % 2 == 0): spinval = 1         #Singlet
            else:  spinval = 2                                                 #Doublet
    ...
    ### Starts printing input 
    with open(paths.inp_path, 'w') as inp:
        print(f"%chk={paths.name}{paths.suffix}.chk", file=inp)
        ...
        ## Functional
        if hasattr(qc_data,"functional"): functional = qc_data.functional.lower()
        ...
        ## Basis
        if hasattr(qc_data,"basis"): basis = qc_data.basis.lower()
        else:                        basis = "def2SVP"
        ...
        ## Jobtype
        if hasattr(qc_data,"jobtype"): jobtype = qc_data.jobtype.lower()
        else:                          jobtype = "scf"
        if hasattr(qc_data,"loose_opt"): loose_opt = qc_data.loose_opt
        else:                            loose_opt = False
        if hasattr(qc_data,"tight_opt"): tight_opt = qc_data.tight_opt
        ...
        ## Grimme
        if hasattr(qc_data,"isGrimme"): isGrimme = qc_data.isGrimme
        else:                           isGrimme = False
        if isGrimme: commandline.append(" EmpiricalDispersion=GD3BJ")
        ...
        ####################################################
        if hasattr(qc_data,"coord_tag"):  coord_tag = qc_data.coord_tag.lower()
...
#def gen_G16_input(mol: object, qc_data: object, paths: object, spin: str='gmol', debug: int=0):
    ...
    mol = comp._job._recipe._object
 
    ### Tries to find spin
    if comp.qc_data.spin == "gmol": 
        if hasattr(mol, 'spin'):
            spinval = 0
            for met in mol.metalist:
                if met.label == "Fe" and mol.spin.lower() == "ls": spinval = spinval + 1
    ...
    elif comp.qc_data.spin == "hs" or comp.qc_data.spin == "ls":
        spinval = 0
        for met in mol.metalist:
            ## I Should add Oxidation-State-dependent Rules
            if met.label == "Fe" and spin.lower() == "ls": spinval = spinval + 1
