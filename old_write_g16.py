...
from Test_V3.Control_Jobs import set_user, set_cluster
...
def gen_G16_input(mol: object, qc_data: object, paths: object, spin: str='gmol', debug: int=0):
    ...
    with open(paths.inp_path, 'w') as inp:
        print(f"%chk={paths.inp_name}.chk", file=inp)
        ...
        ## Functional
        if "functional" in qc_data.keys(): functional = qc_data['functional'].lower()
        ...
        ## Basis
        if "basis" in qc_data.keys(): basis = qc_data['basis'].lower()
        else:                         basis = "def2SVP"
        if basis.lower() == "def2sv":      commandline.append(" def2SV")
        elif basis.lower() == "def2svp":   commandline.append(" def2SVP")
        elif basis.lower() == "def2tzv":   commandline.append(" def2TZV")
        ...
        ## Jobtype
        if "jobtype" in qc_data.keys():   jobtype = qc_data['jobtype'].lower()
        else:                             jobtype = "scf"
        if "loose_opt" in qc_data.keys(): loose_opt = qc_data['loose_opt']
        else:                             loose_opt = False
        if "tight_opt" in qc_data.keys(): tight_opt = qc_data['tight_opt']
        ...
        ## Grimme
        if "isGrimme" in qc_data.keys(): isGrimme = qc_data['isGrimme']
        else:                            isGrimme = False
        if isGrimme: commandline.append(" EmpiricalDispersion=GD3BJ")
        ...
        ####################################################
        if "coord_tag" in qc_data.keys():  coord_tag = qc_data['coord_tag'].lower()
